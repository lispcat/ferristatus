...
- signal :: (TODO) forcefully updates the module and reprints, when it receives the corresponding UNIX real-time signal.
  - given this example, running =$ pkill -SIGRTMIN+6 ferristatus= will forcefully refresh the Alsa module and reprint.
...
- Updating:
  ...
  match signal {
    Some(s) => {
      update corresponding component or print unknown signal for this iteration.
      reuse previous cache but replace one with the newly updated.
    },
    None => {
      update all components.
      collect all cache for all components.
    }
  }
